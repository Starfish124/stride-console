...
export function WeekPeek({ today, entries }: { today: string; entries: Entry[] }) {
  ...
  return <section id="home-week" className="week-peek workspace-panel" aria-labelledby="week-peek-title">
    ...
    <div className="week-days" role="group" aria-label="Choose a day in the next week">{days.map(day => {
      ...
      return <button key={day.iso} type="button" className={cn("week-day",selected === day.iso && "is-selected")} aria-pressed={selected === day.iso} aria-label={`${day.day} ${day.number}, ${count} scheduled ${count === 1 ? "item" : "items"}${day.iso === today ? ", today" : ""}`} onClick={() => {setSelected(day.iso);setExpanded(false);}}><span>{day.day}</span><strong>{day.number}</strong><i className={cn(count > 0 && "has-events")} aria-hidden="true" /></button>;
    })}</div>
    <div className="week-agenda" key={selected}>
